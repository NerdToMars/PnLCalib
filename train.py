...
def parse_args():
    ...
    parser.add_argument(
        "--cfg", type=str, required=True, help="Path to the configuration file"
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        "--batch", type=int, default=1, help="Batch size for train / val (default: 2)"
    )
    parser.add_argument(
        "--num_workers",
        type=int,
    ...
    parser.add_argument(
        "--wandb_project",
        type=str,
        default="soccer-net-calibration",
...
if __name__ == "__main__":
    ...
    if dataset == "SoccerNet":
        ...
        training_set = SoccerNetCalibrationDataset(
            args.root_dir, "train", transform=transforms, main_cam_only=True
        )
        validation_set = SoccerNetCalibrationDataset(
            args.root_dir, "valid", transform=no_transforms, main_cam_only=True
        )
    elif dataset == "WorldCup14":
        from model.transformsWC import transforms, no_transforms

        training_set = WorldCup2014Dataset(
            args.root_dir, "train_val", transform=transforms
        )
        validation_set = WorldCup2014Dataset(
        ...
        training_set = TSWorldCupDataset(args.root_dir, "train", transform=transforms)
    ...
    training_loader = torch.utils.data.DataLoader(
        training_set, num_workers=args.num_workers, batch_size=args.batch, shuffle=True
    )
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, patience=args.patience, mode="min", factor=args.factor
    )
    ...
    for epoch in range(args.num_epochs):
        avg_loss = train_one_epoch(
            epoch + 1, training_loader, optimizer, loss_fn, model, device
        )
        avg_vloss, acc, prec, rec, f1 = validation_step(
            validation_loader, loss_fn, model, device
        )
        scheduler.step(avg_vloss)
        print("LOSS train {} valid {}".format(avg_loss, avg_vloss))
        print(
            f"Acc: {round(acc, 3)}  Prec: {round(prec, 3)}  Rec: {round(rec, 3)}  F1: {round(f1, 3)}"
        )
