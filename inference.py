...
def project(frame, P):
    for line in lines_coords:
        ...
        i2 /= i2[-1]
        frame = cv2.line(
            frame, (int(i1[0]), int(i1[1])), (int(i2[0]), int(i2[1])), (255, 0, 0), 3
        )

    r = 9.15
    pts1, pts2, pts3 = [], [], []
    base_pos = np.array([11 - 105 / 2, 68 / 2 - 68 / 2, 0.0, 0.0])
    for ang in np.linspace(37, 143, 50):
        ang = np.deg2rad(ang)
        pos = base_pos + np.array([r * np.sin(ang), r * np.cos(ang), 0.0, 1.0])
        ipos = P @ pos
        ipos /= ipos[-1]
        pts1.append([ipos[0], ipos[1]])

    base_pos = np.array([94 - 105 / 2, 68 / 2 - 68 / 2, 0.0, 0.0])
    ...
    if input_type == "video":
        ...
        while cap.isOpened():
            ...
            final_params_dict = inference(
                cam, frame, model, model_l, kp_threshold, line_threshold, pnl_refine
            )
    ...
    for batch_idx, batch in enumerate(line_coords):
        ...
        for line_idx, line in enumerate(batch):
            if line is not None:
                ...
                frame = cv2.circle(
                    frame, (x1, y1), radius=5, color=(255, 0, 0), thickness=-1
                )  # Start point (red)
                frame = cv2.circle(
                    frame, (x2, y2), radius=5, color=(0, 0, 255), thickness=-1
                )  # End point (blue)
...
def visualize_keypoints(input_frame, kp_coords, visualize=False):
    ...
    try:
        ...
        for batch_idx, batch in enumerate(kp_coords):
            print(
                f"Processing keypoint batch {batch_idx}, contains {len(batch)} keypoints"
            )
            for kp_idx, kp in enumerate(batch):
                if kp is not None:
                    ...
                    if conf > 0.0000001:  # Adjust threshold as needed
                        print(
                            f"Drawing keypoint {kp_idx} at ({x}, {y}) with confidence {conf}"
                        )
                        # Draw keypoint as a green circle
                        cv2.circle(
                            frame, (x, y), radius=7, color=(0, 255, 0), thickness=-1
                        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process video or image and plot lines on each frame."
    )
    parser.add_argument(
        "--weights_kp", type=str, help="Path to the model for keypoint inference."
    )
    parser.add_argument(
        "--weights_line", type=str, help="Path to the model for line projection."
    )
    parser.add_argument(
        "--kp_threshold",
        type=float,
    ...
    parser.add_argument(
        "--pnl_refine", action="store_true", help="Enable PnL refinement module."
    )
    parser.add_argument(
        "--device", type=str, default="cpu", help="CPU or CUDA device index"
    )
    parser.add_argument(
        "--input_path",
        type=str,
    ...
    parser.add_argument(
        "--save_path", type=str, default="", help="Path to save the processed video."
    )
    parser.add_argument(
        "--display", action="store_true", help="Enable real-time display."
    )
    ...
    model = get_cls_net(cfg, device=device)
