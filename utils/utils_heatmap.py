...
def generate_gaussian_array_vectorized(
    num_matrices, keypoints, original_size, down_ratio=2, sigma=2, proj_err_th=5.0
):
    ...
    matrices = [
        generate_gaussian_matrix_vectorized(new_size[0], new_size[1], px, py, sigma)
...
def generate_gaussian_array_vectorized_l(
    num_matrices, keypoints, original_size, down_ratio=2, sigma=2, sigma_mult=1
):
    ...
    matrices1 = [
        generate_gaussian_matrix_vectorized(
            new_size[0], new_size[1], px, py, sigma_f(px, py, new_size, sigma)
        )
        for px, py in center_points[::2]
    ]
    matrices2 = [
        generate_gaussian_matrix_vectorized(
            new_size[0], new_size[1], px, py, sigma_f(px, py, new_size, sigma)
        )
        for px, py in center_points[1::2]
    ]
...
def complete_keypoints(kp_dict, lines_dict, w, h, normalize=False):
    ...
    for key in range(1, 31):
        if key not in kp_dict.keys():
            ...
            line_key1, line_key2 = (
                lines_list.index(line_keys[0]) + 1,
                lines_list.index(line_keys[1]) + 1,
            )
            if all(
                line_key in lines_dict.keys() for line_key in [line_key1, line_key2]
            ):
                ...
                if (
                    -w_extra < new_kp[0] < w_extra + w
                    and -h_extra < new_kp[1] < h_extra + h
                ):
                    complete_dict[key] = {
                        "x": round(new_kp[0], 0),
                        "y": round(new_kp[1], 0),
