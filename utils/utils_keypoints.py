...
class CourtDefinition(NamedTuple):
    ...
    def get_keypoint_world_coords_2D(
        self,
    ) -> Tuple[List[str], List[Tuple[float, float]]]:
        """
        Generate key points on a basketball half-court with the basket at the origin (0,0).

        Returns:
            Tuple of (List of names, List of (x, y) coordinates)

            rim_center: (0, 0)
            left_corner: (-750.0, 160)
            right_corner: (750.0, 160)
            three_point_limit_left: (-660.0, 160)
            three_point_limit_right: (660.0, 160)
            left_end_of_free_throw_line_in_free_throw_circle: (-180.0, -575.0)
            right_end_of_free_throw_line_in_free_throw_circle: (180.0, -575.0)
            left_end_of_three_point_arc: (-660.0, -141.50971698084905)
            right_end_of_three_point_arc: (660.0, -141.50971698084905)
        """
        key_points = []
        key_points_name = []
        # Extract dimensions from court definition
        # this we do because the court is rotated 90 degrees
        width = self.height
        # height = self.height

        circle_d = self.circle_diameter
        three_point_dist = self.three_point_distance
        three_point_limit = self.three_point_limit
        key_area_length = self.key_area_length

        # x distance between center of court and three-point limit
        center_to_three_point_limit_distance_x = (width / 2) - three_point_limit
        assert center_to_three_point_limit_distance_x > 0

        # Calculate baseline position (from basket to baseline)
        baseline_y = self.rim_center_offset

        # rim center
        key_points.append((0, 0))
        key_points_name.append("rim_center")

        # Left corner: intersection between left side line and baseline
        key_points.append((-width / 2, baseline_y))
        key_points_name.append("left_corner")

        # Right corner: intersection between right side line and baseline
        key_points.append((width / 2, baseline_y))
        key_points_name.append("right_corner")

        # center on baseline
        key_points.append((0, baseline_y))
        key_points_name.append("center_on_baseline")

        # Three-point limit left on baseline
        key_points.append((-center_to_three_point_limit_distance_x, baseline_y))
        key_points_name.append("three_point_limit_left")

        # Three-point limit right on baseline
        key_points.append((center_to_three_point_limit_distance_x, baseline_y))
        key_points_name.append("three_point_limit_right")

        # Left end of circle-d
        key_points.append((-circle_d / 2, -key_area_length + self.rim_center_offset))
        key_points_name.append("left_end_of_free_throw_line_in_free_throw_circle")

        # Right end of circle-d
        key_points.append((circle_d / 2, -key_area_length + self.rim_center_offset))
        key_points_name.append("right_end_of_free_throw_line_in_free_throw_circle")

        # center on free throw line
        ...
        if center_to_three_point_limit_distance_x <= three_point_dist:
            ...
            key_points.append(
                (-center_to_three_point_limit_distance_x, -y_intersection_to_rim_center)
            )
            key_points_name.append("left_end_of_three_point_arc")

            # Intersection between three-point limit and arc, right side
            key_points.append(
                (center_to_three_point_limit_distance_x, -y_intersection_to_rim_center)
            )
            key_points_name.append("right_end_of_three_point_arc")

        # center point on arc
        key_points.append((0, -three_point_dist))
        key_points_name.append("center_on_arc")

        # 45 degree point on arc
        key_points.append(
            (
                (three_point_dist / np.sqrt(2)).item(),
                (-three_point_dist / np.sqrt(2)).item(),
            )
        )
        key_points_name.append("45_degree_point_on_arc")

        # 135 degree point on arc
        key_points.append(
            (
                -three_point_dist / np.sqrt(2).item(),
                -three_point_dist / np.sqrt(2).item(),
        ...
        key_points_name.append("135_degree_point_on_arc")

        return key_points_name, key_points
...
class BasketballCourtKeypointsDB(object):
    def __init__(self, input_court_definition: str):
        ...
        self.keypoint_pair_list = [
            ...
            ["baseline", "Side line right"],
            ["baseline", "three point line left"],
            ["baseline", "three point line right"],
        ]

        self.keypoint_aux_pair_list = [
            ["three point line left", "free throw line"],
            ["three point line right", "free throw line"],
            ["free throw line", "side line left"],
            ["free throw line", "side line right"],
        ]

        self.keypoint1_pair_list = [
            ["arc", "three point line left"],
            ["arc", "three point line right"],
        ]

        self.keypoint3_check_dict = {
            "arc": [
                "center_on_arc",
                "45_degree_point_on_arc",
                "135_degree_point_on_arc",
            ],
            "arc center": ["rim_center"],
            ...
            if num_proposals + len(obj_list[0]) >= 4:
                ...
                for comb in filtered_combinations:
                    if sanity_check(candidates_set, comb):
                        obj_list_aux, img_list_aux = (
                            copy.deepcopy(obj_list),
                ...
                final_comb = (
                    min_rep_err_comb
                    if min_rep_err_comb
                    else min_rep_err_sub_comb
                    if min_rep_err_sub_comb
                    else None
                )
